...
refreshNutritionTab() {
        ...
        // Clinical Warnings logic
        const warningsContainer = document.getElementById('clinical-nutrition-warnings');
        if (warningsContainer) {
            ...
            if (profile.sex === 'female') {
                warnings.push(`
                    <div class="clinical-warning-item">
                        <strong>🛡️ Preservação Estrutural</strong>
                        Alerta de adesão rígida: Procure consumir de 20 a 30g de proteína por refeição para evitar perda de massa magra.
                    </div>
                `);
            }

            // Age Alert (Leucine Threshold)
            if (profile.age >= 65) {
                warnings.push(`
                    <div class="clinical-warning-item">
                        <strong>⚡ Limiar de Leucina</strong>
                        Como idoso, suas refeições devem ter pelo menos 25-30g de proteína para ativar a síntese muscular efetivamente.
                    </div>
                `);
            }
