...
// --- JOURNEY V12 ---
    ...
    if (comparison && comparison.start) {
        ...
        if (imgStart && startUrl) imgStart.src = startUrl;
        if (dateStart) dateStart.textContent = DateService.format(comparison.start.dateISO, 'DD/MM');
    }
    if (comparison && comparison.current) {
        const currentUrl = await JourneyService.getPhotoUrl(comparison.current.id);
        if (imgCurrent && currentUrl) imgCurrent.src = currentUrl;
        if (dateCurrent) dateCurrent.textContent = DateService.format(comparison.current.dateISO, 'DD/MM');
    }
