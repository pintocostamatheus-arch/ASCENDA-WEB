...
const SymptomsController = {
    ...
    addCustomSymptomRow() {
        ...
        if (name && name.trim()) {
            SymptomsService.saveCustomName(SecurityUtils.sanitizeHTML(name.trim()));
            this.refreshSymptomsTab();
        }
    },

    deleteCustomSymptom(name, event) {
        if (event) event.stopPropagation();
        if (confirm(`Remover sintoma personalizado "${name}"?`)) {
            let names = SymptomsService.getCustomNames();
            names = names.filter(n => n !== name);
            localStorage.setItem(SymptomsService.CUSTOM_NAMES_KEY, JSON.stringify(names));
            delete this.symptomValues[name];
            this.refreshSymptomsTab();
        }
    },
    ...
    saveSymptoms() {
        ...
        try {
            ...
            // Map custom symptoms to an array
            const customArray = [];
            SymptomsService.getCustomNames().forEach(name => {
                if (this.symptomValues[name] > 0) {
                    customArray.push({ name: name, value: this.symptomValues[name] });
                }
            });

            // Isolate built-in symptoms
            const builtInValues = {};
            SymptomsService.SYMPTOMS.forEach(s => {
                builtInValues[s] = this.symptomValues[s] || 0;
            });
    ...
    finalSaveSymptoms(data) {
        ...
        Object.keys(this.symptomValues).forEach(k => this.symptomValues[k] = 0);
