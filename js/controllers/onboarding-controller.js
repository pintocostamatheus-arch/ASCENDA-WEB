...
handleWizardNext() {
    ...
    if (this.validateStep(this.currentWizardStep)) {
        if (this.currentWizardStep === totalSteps) {
            ...
            // Fix: Trigger calc when entering Step 7 (Summary)
...
goToStep(step) {
    ...
    const safeStep = Math.max(0, Math.min(step, 7));
    // 0 to 7 = 8 steps. 100% / 8 steps? No, simple translateX works best with 100% width steps
    ...
    if (progressBar) {
        // Step 0 = 0%, Step 1 = 14% ...
        const progress = (safeStep / 7) * 100;
        progressBar.style.width = `${progress}%`;
    }

    this.updateWizardButtons(safeStep, 7);
},
